...
// param 可以是log配置文件名，也可以是log配置内容,默认DEBUG输出到控制台
func SetNewLogger(newLogger *LocalLogger, param ...string) error {
	if 0 == len(param) {
		//默认只输出到控制台
		newLogger.SetLogger(AdapterConsole)
		return nil
	}
	...
	if conf.TimeFormat != "" {
		newLogger.timeFormat = conf.TimeFormat
	}
	if conf.Console != nil {
		console, _ := json.Marshal(conf.Console)
		newLogger.SetLogger(AdapterConsole, string(console))
	}
	if conf.File != nil {
		file, _ := json.Marshal(conf.File)
		newLogger.SetLogger(AdapterFile, string(file))
	}
	if conf.Conn != nil {
		conn, _ := json.Marshal(conf.Conn)
		newLogger.SetLogger(AdapterConn, string(conn))
	}
	return nil
}
